...
@app.get("/api/me")
def me():
    u = current_user(); ok = bool(u and u.is_active)
    return jsonify({"authenticated": ok, "user": user_payload(u) if ok else None})
...
def register():
    ...
    u=User(username=username, full_name=f"{first} {last}".strip(), email=email, password_hash=generate_password_hash(password), role="student")
    db.session.add(u); db.session.commit(); session.clear(); session["user_id"]=u.id
...
@admin_required
def dashboard():
    today=date.today(); cutoff=today+timedelta(days=2); rows=Booking.query.filter(Booking.status.in_(("reserved","checked_out")),Booking.due_date<=cutoff).order_by(Booking.due_date,Booking.id).all()
    return jsonify([{**b.to_dict(),"days_overdue":max(0,(today-b.due_date).days)} for b in rows])

@app.get("/api/borrowers/")
@app.get("/api/users")
@admin_required
def borrowers(): return jsonify([user_payload(u) for u in User.query.filter_by(is_active=True).order_by(User.full_name).all()])
...
def transfer(booking_id):
    ...
    if not b:return jsonify({"detail":"Booking not found."}),404
    if not target or not target.is_active:return jsonify({"detail":"Select a valid new borrower."}),400
